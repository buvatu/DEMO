...
import { getTechStandards, insertTechSpec } from '../../services';

class TechSpecAdd extends Component {
  constructor(props) {
    super(props);
    this.state = {
      techSpecID: '',
  ...
  save = async () => {
    const { techSpecID, techSpecName, techStandardList, standardIDErrors } = this.state;
    ...
    if (techSpecID.trim() === '') {
      hasError = true;
      this.setState({ techSpecIDErrorMessage: 'Mã thông số kĩ thuật không thể bỏ trống' });
    }
    if (techSpecName.trim() === '') {
      hasError = true;
      this.setState({ techSpecNameErrorMessage: 'Tên thông số kĩ thuật không thể bỏ trống' });
    }
    ...
    setLoading(true);
    const getInsertTechStandardsResult = await insertTechSpec(techSpecID, techSpecName, techStandardList, auth.userID);
    setLoading(false);
...
export default connect(mapStateToProps, mapDispatchToProps)(TechSpecAdd);
