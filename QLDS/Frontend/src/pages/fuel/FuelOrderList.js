...
import { getFuelOrderList, getUserList } from '../../services';

class FuelOrderList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      status: '',
  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      const getFuelOrderListResult = await getFuelOrderList(auth.role, auth.userID);
      const fuelOrderList = getFuelOrderListResult.data;
      let searchResultList = JSON.parse(JSON.stringify(fuelOrderList));
      ...
      this.setState({
        ...
        fuelOrderList,
        searchResultList,
        fuelOrderListDisplay: searchResultList.slice(0, pageSize),
        statusList: [
  ...
  findFuelOrderList = async () => {
    const { status, requestor, tester, approver, requestDate, pageSize, fuelOrderList } = this.state;
    let searchResultList = [];
    if (status !== '') {
      searchResultList = fuelOrderList.filter((e) => e.status === status);
    }
    if (requestor !== '') {
            ...
            <div className="bx--col-lg-12">
              <TableContainer>
                <Table>
                  ...
                  <TableBody>
                    {fuelOrderListDisplay.map((order, index) => (
                      <TableRow key={`row-${index.toString()}`}>
                        <TableCell>
                          <Link
                            to={{
                              pathname: (order.fuelOrderType === 'I' ? '/fuel/stock-in/' : '/fuel/stock-out/').concat(this.getFuelOrderPath(order)),
                        ...
                        <TableCell>{order.fuelOrderType === 'I' ? 'Yêu cầu nhập kho' : 'Yêu cầu xuất kho'}</TableCell>
                        <TableCell>{order.requestor}</TableCell>
                        <TableCell>{order.fuelOrderNote}</TableCell>
                        <TableCell>{order.requestDate}</TableCell>
                        <TableCell>{order.tester}</TableCell>
                        <TableCell>{order.approver}</TableCell>
                        <TableCell>
                          {order.status === 'created' && 'Chờ nghiệm thu'}
              ...
              <Pagination
                ...
                onChange={(target) => {
                  this.setState({
                    fuelOrderListDisplay: searchResultList.slice((target.page - 1) * target.pageSize, target.page * target.pageSize),
                    page: target.page,
...
export default connect(mapStateToProps, mapDispatchToProps)(FuelOrderList);
